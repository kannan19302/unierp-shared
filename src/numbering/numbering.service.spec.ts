...
function makeSeq(
  overrides: Partial<DocumentSequenceRow> = {},
): DocumentSequenceRow {
  return {
    id: "seq-1",
    tenantId: "tenant-1",
...
describe("NumberingService", () => {
  ...
  describe("getNextNumber", () => {
    ...
    it("throws when sequence is not found", async () => {
      ...
      await expect(
        svc.getNextNumber(tx, "tenant-1", "UNKNOWN"),
      ).rejects.toThrow("DocumentSequence not found");
    });
    ...
    it("passes organizationId to the lookup", async () => {
      ...
      await svc.getNextNumber(tx, "tenant-1", "INV", {
