...
describe("errorEnvelopeSchema (G.9)", () => {
  ...
  it("accepts the canonical envelope with and without details", () => {
    expect(errorEnvelopeSchema.parse(valid)).toMatchObject({
      code: "NOT_FOUND",
    });
    expect(
      errorEnvelopeSchema.parse({ ...valid, errors: [{ field: "name" }] })
        .errors,
    ).toBeDefined();
  });
  ...
  it("rejects non-error status codes and malformed timestamps", () => {
    expect(
      errorEnvelopeSchema.safeParse({ ...valid, statusCode: 200 }).success,
    ).toBe(false);
    expect(
      errorEnvelopeSchema.safeParse({ ...valid, statusCode: 600 }).success,
    ).toBe(false);
    expect(
      errorEnvelopeSchema.safeParse({ ...valid, timestamp: "yesterday" })
        .success,
    ).toBe(false);
    expect(errorEnvelopeSchema.safeParse({ ...valid, code: "" }).success).toBe(
      false,
    );
  });
...
describe("listQuerySchema (G.9)", () => {
  it("applies defaults and coerces strings (query-string reality)", () => {
    ...
    expect(
      listQuerySchema.parse({ page: "3", limit: "50", sortOrder: "desc" }),
    ).toMatchObject({
      page: 3,
      limit: 50,
      sortOrder: "desc",
  ...
  it("rejects out-of-range and malformed values", () => {
    ...
    expect(
      listQuerySchema.safeParse({ limit: LIST_LIMIT_MAX + 1 }).success,
    ).toBe(false);
    expect(listQuerySchema.safeParse({ limit: "lots" }).success).toBe(false);
    expect(listQuerySchema.safeParse({ sortOrder: "sideways" }).success).toBe(
      false,
    );
...
describe("pagination meta + response (G.9)", () => {
  ...
  it("paginatedResponseSchema validates data against the item schema", () => {
    ...
    expect(
      schema.safeParse({ data: [], meta: { ...meta, total: -1 } }).success,
    ).toBe(false);
  });
});
